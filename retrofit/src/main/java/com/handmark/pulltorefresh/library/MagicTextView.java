...
import com.handmark.pulltorefresh.library.PullToRefreshScrollView.ScrollListener;
import com.zizizizizi.retrofitdemo.BaseParam;
...
public class MagicTextView extends TextView implements ScrollListener {
    ...
    private boolean doPlus(int scroll) {
        if (isShown() && (scroll + BaseParam.mWinheight > locHeight + OFFSET)
    ...
    private boolean doMinus(int scroll) {
        ...
        if (isShown() && (scroll + BaseParam.mWinheight - mHeight < locHeight - OFFSET)
